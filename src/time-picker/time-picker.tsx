...
const TimePicker = React.forwardRef<HTMLInputElement, TimePickerProps>(
  (props, ref) => {
    ...
    const setSelectionPositionToInput = () => {
      if (htmlProps.isReadOnly || htmlProps.isDisabled) return
    ...
    const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      const currentNumber = Number(
        `${currentValueArr[selectedSelectionInput.start]}${currentValueArr[selectedSelectionInput.end - 1]
        }`,
      )
    ...
    const handleKeyDownWithNumberKey = (
      ...
      if (
        ...
        replaceText = `${currentValueArr[selectedSelectionInput.end - 1] ?? '0'
          }${key}`
    ...
    const handleIconClockClick = (
      ...
      event.stopPropagation()
      event.preventDefault()
      if (htmlProps.isDisabled) return
      setIsOpen((isOpen) => {
        onOpenChange?.(!isOpen)
        return !isOpen
      })
      ...
      >
        ...
        >
          <TimePickerDropdown
            ...
            hasFooter={hasFooter}
            isReadOnly={!!htmlProps.isReadOnly}
